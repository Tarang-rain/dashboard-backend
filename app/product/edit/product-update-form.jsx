...
const ProductUpdateForm = ({ productData }) => {
  ...
  useEffect(() => {
    if (productData) {
      reset({
        ...
        product_stock: productData.product_stock?.toString() || "",
        product_discount: productData.product_discount?.toString() || "",
      });

      // Set tags
      if (productData.product_tags && Array.isArray(productData.product_tags)) {
        setTags(productData.product_tags);
        setValue("product_tags", productData.product_tags.join(","));
      }

      // Set variants
      if (
        productData.product_variants &&
        Array.isArray(productData.product_variants) &&
  ...
  const handleImageChange = (e) => {
    ...
    if (existingImages.length + imageUrls.length + files.length > 5) {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const allImages = [...existingImages.map((img) => img.url), ...imageUrls];
      ...
      const response = await fetch(
        `/api/update-product/${productData.product_id}`,
        {
          method: "PUT",
          headers: {
        ...
        <Card.Body>
          ...
          <Form onSubmit={handleSubmit(onSubmit)}>
            ...
            <input
              type="hidden"
              {...register("product_id")}
              defaultValue={productData?.product_id}
            />
            ...
            <Form.Group className="mb-3">
              ...
              <div className="mt-2 d-flex flex-wrap">
                {tags.map((tag, index) => (
                  <div
                    className="me-2 mb-2 d-flex align-items-center bg-light rounded"
                    key={index}
              ...
              <div className="d-flex flex-wrap mt-2 gap-2">
                {imageUrls.map((url, index) => (
                  ...
                  >
                    ...
                    <span className="position-absolute bottom-0 start-0 bg-info text-white px-2 py-1 small">
                      New
                    </span>
                  </div>
                ))}
